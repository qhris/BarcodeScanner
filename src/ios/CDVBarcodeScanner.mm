...
//    self.previewLayer = nil;
    ...
    [self performSelector:@selector(openDialog) withObject:nil afterDelay:2];
}
...
- (void)flipCamera
{
    if (self.captureSession) {
        ...
        NSArray* previousInputs = [self.captureSession inputs];
        [previousInputs enumerateObjectsUsingBlock:^(AVCaptureInput *obj, NSUInteger idx, BOOL *stop) {
            [self.captureSession removeInput:obj];
        }];
        ...
        if (self.isFrontCamera) {
            ...
            if (!device) NSLog(@"unable to obtain video capture device");

        }

        NSError* error = nil;
        AVCaptureDeviceInput* input = [AVCaptureDeviceInput deviceInputWithDevice:device error:&error];
        if (!input) NSLog(@"unable to obtain video capture device input");
    ...
    *ptr = greyData;
    ...
    for (size_t i=0; i<greyWidth; i++) {
        for (size_t j=0; j<greyWidth; j++) {
            ...
            // increase scan contrast. makes it easier to scan codes
            // on lighter backgrounds.
            value = (value * value) / 256.0;
...
- (BOOL)shouldAutorotate
{
    ...
    UIInterfaceOrientation targetOrientation = [UIDevice currentDevice].orientation;
    return [supportedOrientations containsObject:NSStringFromUIInterfaceOrientation(targetOrientation)];
}

// Deprecated in iOS 6 and onwards...
- (BOOL)shouldAutorotateToInterfaceOrientation:(UIInterfaceOrientation)interfaceOrientation
{
